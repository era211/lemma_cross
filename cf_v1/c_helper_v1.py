...
def forward_ab(parallel_model, ab_dict, c_only_dict, e_only_dict, device, indices, lm_only=False):
    # ff
    ...
    # c_only
    c_only_batch_tensor_ab = c_only_dict['input_ids'][indices, :]
    c_only_batch_am_ab = c_only_dict['attention_mask'][indices, :]
    c_only_batch_posits_ab = c_only_dict['position_ids'][indices, :]
    # am_g_ab, arg1_ab, arg2_ab = get_arg_attention_mask(batch_tensor_ab, c_only_parallel_model)

    # 获取 <mask> token 的 ID
    c_only_mask_token_id = parallel_model.module.tokenizer.mask_token_id
    # 生成 mask_attention: 如果 token 不是 <mask>，则为 1，否则为 0
    c_only_mask_attention = (c_only_batch_tensor_ab != c_only_mask_token_id).long()

    c_only_batch_tensor_ab.to(device)
    c_only_batch_am_ab.to(device)
    c_only_batch_posits_ab.to(device)
    if c_only_mask_attention is not None:
        c_only_mask_attention.to(device)

    # e_only
    e_only_batch_tensor_ab = e_only_dict['input_ids'][indices, :]
    e_only_batch_am_ab = e_only_dict['attention_mask'][indices, :]
    e_only_batch_posits_ab = e_only_dict['position_ids'][indices, :]

    e_only_batch_tensor_ab.to(device)
    e_only_batch_am_ab.to(device)
    e_only_batch_posits_ab.to(device)

    return parallel_model(batch_tensor_ab, batch_am_ab, batch_posits_ab, am_g_ab, arg1_ab, arg2_ab,
                          c_only_batch_tensor_ab, c_only_batch_am_ab, c_only_batch_posits_ab, c_only_mask_attention,
...
def save_parameters(scorer_folder, parallel_model):
    model_path = scorer_folder + 'CrossEncoder/'
